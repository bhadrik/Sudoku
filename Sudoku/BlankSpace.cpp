...
void BlankSpace::finalProcess(short sudoku[9][9]) {
...
bool BlankSpace::isFilled() {
	return status;
}

void BlankSpace::changStatus(bool newStatus) {
	status = newStatus;
}

void BlankSpace::printMask() {
	cout << "---";
	for (int i = 1; i < 10; i++)
		cout << mask[i];
	cout << endl;
...
void BlankSpace::forceApply(short sudoku[9][9], short forceApplicableNumber) {
	applicableNumber = forceApplicableNumber;
	apply(sudoku);
}

void BlankSpace::manualProcess(short sudoku[9][9]) {
	short a = 0, b = 0;
	//row
...
void BlankSpace::countPossibleNumbers() {
	possibleNumbers = 0;
...
void BlankSpace::apply(short sudoku[9][9]) {
